...
このコネクタは Microsoft 脅威インテリジェンス インジケーターを `ThreatIntelligenceIndicator` テーブルに自動的に取り込みます。
...
1. `Microsoft Defender Thread Intelligence` を検索して開く
...
これらの TI フィードのいずれかから取り込まれた 侵害の兆候 (Indicators of Compromise, IOC) は、 `ThreatIntelligenceIndicator` と呼ばれる専用のテーブルに格納され、左側のナビゲーション メニューの [脅威インテリジェンス] メニューに表示されます。

1. [全般]-[ログ] を開く

    ![](../images/ex07-201.png)

1. 以下のクエリで `ThreatIntelligenceIndicator` テーブルのスキーマを確認
...
TI データを `ThreatIntelligenceIndicator` テーブルに取り込んだ後、私たちの使命は、SOC が TI メニューをどのように活用して管理し、IOC のライフサイクルを検索、タグ付け、管理できるかを確認することです。
...
1. 検索条件を確認
    ...
    メインブレードの上部領域では、特定のパラメーターに基づいてIOCリストをフィルタリングできます。
...
1. [脅威管理]-[脅威インテリジェンス] を開き、「新規追加」を選択
...
1. 以下のクエリを実行し、作成したIOCが `ThreatIntelligenceIndicator ` テーブルに追加されていることを確認
...
1. [構成]-[分析] を開き、「規則のテンプレート」タブへ移動

    ![](../images/ex07-301.png)

1. 「フィルターの追加」を開き、以下の条件でフィルタ

    - データソース: `Microsoft Defender Threat Inteligence`
...
1. TI一覧を確認
    ...
    これらはすべて、さまざまなデータ ソースを TI テーブル (`ThreatIntelligenceIndicator`) に存在する IOC と関連付けて、組織のログ内の悪意のある侵害インジケーターのトレースを検出します。
...
1. 「視覚化」を開いて「棒グラフ」を選択、左上「編集完了」を選択して保存
