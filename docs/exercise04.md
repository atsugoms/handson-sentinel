...
1. [脅威管理]-[インシデント] を開く
    ...
    インシデントページはデフォルトで過去 24 時間以内に発生したすべてのインシデントが表示されます。

1. フィルター条件を利用することで インシデントがフィルタリングできることを確認
...
1. `Sign-ins from IPs that attempt sign-ins to disabled accounts` を検索して選択

1. 右ペインの所有者を取得し、ステータスをアクティブに変更
...
1. 「概要」の「インシデントのタイムライン」から最新インシデントを選択

1. 「イベント」にある「LA (Log Analytics) にリンク」を開く
...
1. 「マイ ブック」タブを開き、 `Investigation Insights` を選択

1. 右ペインから「保存されたブックの表示」を開く
...
1. 「Entity Insights」の「Investigate IP Address」に調査で判明したアクセス元IPアドレス `175.45.176.99` を入力
...
1. `Solorigate Inventory check` を探して選択

1. 右ペインから「クエリの実行」を押下

1. 右ペイン上部の「結果」に数値 (`3`) が出ていることを確認
...
1. `Solorigate Network Beacon` インシデントを選択、
    関連する IPアドレス (`17.81.146.1`) をコピー
